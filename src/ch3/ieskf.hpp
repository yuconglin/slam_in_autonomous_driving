...
bool IESKF<S>::ObserveWheelSpeed(const Odom& odom) {
    ...
    // 卡尔曼增益
    const Eigen::Matrix<S, 18, 3> K = cov_ * H.transpose() * (H * cov_ * H.transpose() + odom_noise_).inverse();
    ...
    const double average_vel = 0.5 * (velo_l + velo_r);

    const VecT vel_odom(average_vel, 0.0, 0.0);

    for (int iter = 0; iter < options_.num_iterations_; ++iter) {
        dx_ = K * (R_ * vel_odom - v_);
        UpdateAndReset();
        if (dx_.norm() < options_.quit_eps_) {
            LOG(INFO) << "speedometer observation converged at iteration " << iter;
            break;
        }
    }
...
template <typename S>
bool IESKF<S>::ObserveGps(const GNSS& gnss) {
    /// GNSS 观测的修正
...
bool IESKF<S>::ObserveSE3(const SE3& pose, double trans_noise, double ang_noise) {
    ...
    const Mat6d V = noise_vec.asDiagonal();
    const Eigen::Matrix<S, 18, 6> K = cov_ * H.transpose() * (H * cov_ * H.transpose() + V).inverse();
    ...
    for (int iter = 0; iter < options_.num_iterations_; ++iter) {
        ...
        UpdateAndReset();
        if (dx_.norm() < options_.quit_eps_) {
            LOG(INFO) << "SE3 observation converged at iteration " << iter;
            break;
        }
    }
...
#endif  // SLAM_IN_AUTO_DRIVING_IESKF_HPP
