...
#ifndef SLAM_IN_AUTO_DRIVING_EUKF_HPP
#define SLAM_IN_AUTO_DRIVING_EUKF_HPP
 ...
 *
 * 本书使用18维的EUKF，标量类型可以由S指定，默认取double
...
class EUKF {
   public:
     ...
     * 初始零偏取零
     */
    EUKF(Options option = Options()) : options_(option) { BuildNoise(option); }
   ...
   private:
    ...
    /// Predicts given a full initial state.
...
using EUKFD = EUKF<double>;
using EUKFF = EUKF<float>;

template <typename S>
bool EUKF<S>::Predict(const IMU& imu) {
    ...
    const Mat18T F_t = F.transpose();

    std::vector<Vec18T> sigma_points(2 * kDim + 1);
    sigma_points[0].setZero();
    // Cholesky Decomposition, L lower-triangular
    const Mat18T L(cov_.llt().matrixL());
    const double sqrt_dim = std::sqrt(kDim + options_.kappa_);
    for (int i = 1; i <= kDim; ++i) {
        sigma_points[i] = sqrt_dim * L.col(i - 1);
        sigma_points[i + kDim] = -sigma_points[i];
    }
    // The predicted dx_ will still be zero because the prediction equation is linear.
    // We can still use sigma points to compute the predicted covariance matrix.
    // dx_ = F * dx_ doesn't include motion model's noise. We have to include it here in covariance matrix prediction.
    cov_ = Q_;
    const double alpha = 0.5 * 1.0 / (kDim + options_.kappa_);
    for (int i = 1; i <= 2 * kDim; ++i) {
        cov_ += alpha * F * sigma_points[i] * sigma_points[i].transpose() * F_t;
    }
    current_time_ = imu.timestamp_;
    return true;
}
...
template <typename S>
bool EUKF<S>::ObserveWheelSpeed(const Odom& odom) {
    ...
    const VecT vel_world = R_ * vel_odom;
    ...
    Mat3T Sigma_yy = odom_noise_;
    Eigen::Matrix<S, 18, 3> Sigma_xy = Eigen::Matrix<S, 18, 3>::Zero();
    const double alpha = 0.5 * 1.0 / (kDim + options_.kappa_);
    // sigma_points are disturb to v_ and the measurement errors are sigma_points.
    for (int i = 1; i <= 2 * kDim; ++i) {
        const VecT error = sigma_points[i].template block<3, 1>(3, 0);
        Sigma_yy += alpha * error * error.transpose();
        Sigma_xy += alpha * sigma_points[i] * error.transpose();
    }

    const Eigen::Matrix<S, 18, 3> K = Sigma_xy * Sigma_yy.inverse();
    cov_ -= K * Sigma_xy.transpose();
    dx_ = K * (vel_world - v_);
...
template <typename S>
bool EUKF<S>::ObserveGps(const GNSS& gnss) {
    /// GNSS 观测的修正
...
template <typename S>
bool EUKF<S>::ObserveSE3(const SE3& pose, double trans_noise, double ang_noise) {
    /// 既有旋转，也有平移
    ...
    // The averaged predicted measurement is (p_, R_) because the averaged disturbs from sigma points are zero.

    Eigen::Matrix<S, 6, 6> Sigma_yy = V;
    Eigen::Matrix<S, 18, 6> Sigma_xy = Eigen::Matrix<S, 18, 6>::Zero();
    const double alpha = 0.5 * 1.0 / (kDim + options_.kappa_);
    // sigma_points[0] is a zero vector and therefore we can safely start from 1.
    for (int i = 1; i <= 2 * kDim; ++i) {
        // The error of predicted measurements for each sigma points will be equal to sigma point itself due to
        // measurement equations.
        Vec6d error = Vec6d::Zero();
        error.template head<3>() = sigma_points[i].template block<3, 1>(0, 0);
        error.template tail<3>() = sigma_points[i].template block<3, 1>(6, 0);
        Sigma_yy += alpha * error * error.transpose();
        // Here sigma points are state's disturbs too.
        Sigma_xy += alpha * sigma_points[i] * error.transpose();
    }

    const Eigen::Matrix<S, 18, 6> K = Sigma_xy * Sigma_yy.inverse();
    cov_ -= K * Sigma_xy.transpose();
    dx_ = K * innov;
...
#endif  // SLAM_IN_AUTO_DRIVING_EUKF_HPP
