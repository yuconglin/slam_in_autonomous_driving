...
void KdTreeTemp::Insert(const IndexVec &points, KdTreeTempNode *node) {
    ...
    const auto create_if_not_empty = [&node, this](KdTreeTempNode *&new_node, const IndexVec &index) {
        if (!index.empty()) {
            new_node = new KdTreeTempNode;
...
void KdTreeTemp::Knn(const Vec3f &pt, KdTreeTempNode *node,
                     std::priority_queue<NodeAndDistanceTemp> &knn_result) const {
    if (node->IsLeaf()) {
        // 如果是叶子，检查叶子是否能插入
    ...
    // 然后再看另一侧子树是否需要搜索
    KdTreeTempNode *this_side, *that_side;
...
bool KdTreeTemp::NeedExpand(const Vec3f &pt, KdTreeTempNode *node,
                            std::priority_queue<NodeAndDistanceTemp> &knn_result) const {
...
void KdTreeTemp::ComputeDisForLeaf(const Vec3f &pt, KdTreeTempNode *node,
                                   std::priority_queue<NodeAndDistanceTemp> &knn_result) const {
...
bool KdTreeTemp::FindSplitAxisAndThresh(const IndexVec &point_idx, int &axis, float &th, IndexVec &left,
                                        IndexVec &right) {
...
void KdTreeTemp::Reset() {
    tree_node_id_ = 0;
    root_.reset(new KdTreeTempNode());
    root_->id_ = tree_node_id_++;
    size_ = 0;
}

void KdTreeTemp::Clear() {
