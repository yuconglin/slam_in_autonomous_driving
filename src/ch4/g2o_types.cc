...
void EdgeInertial::computeError() {
    ...
    const Vec3d ep =
        RiT * (p2->estimate().translation() - p1->estimate().translation() - v1->estimate() * dt_ - grav_ * dt2_ / 2) -
        dp;
    _error << er, ev, ep;
}
...
EdgeInertialNew::EdgeInertialNew(std::shared_ptr<IMUPreintegration> preinteg, const Vec3d& gravity, double weight)
    : preint_(preinteg), dt_(preinteg->dt_), dt2_(preinteg->dt2_) {
    resize(6);  // 6个关联顶点
    grav_ = gravity;
    setInformation(preinteg->cov_.inverse() * weight);
...
void EdgeInertialNew::computeError() {
    auto* p1 = dynamic_cast<const VertexPose*>(_vertices[0]);
    auto* v1 = dynamic_cast<const VertexVelocity*>(_vertices[1]);
    auto* bg1 = dynamic_cast<const VertexGyroBias*>(_vertices[2]);
    auto* ba1 = dynamic_cast<const VertexAccBias*>(_vertices[3]);
    ...
    const Vec3d er = (p2->estimate().so3().inverse() * p1->estimate().so3() * dR).log();
    const Mat3d Ri = p1->estimate().so3().matrix();
    const Vec3d ev = Ri * dv + v1->estimate() + grav_ * dt_ - v2->estimate();
    const Vec3d ep = Ri * dp + p1->estimate().translation() + v1->estimate() * dt_ + 0.5 * grav_ * dt2_ -
                     p2->estimate().translation();

    _error << er, ev, ep;
}

void EdgeInertialNew::linearizeOplus() {
    auto* p1 = dynamic_cast<const VertexPose*>(_vertices[0]);
    auto* v1 = dynamic_cast<const VertexVelocity*>(_vertices[1]);
    auto* bg1 = dynamic_cast<const VertexGyroBias*>(_vertices[2]);
    auto* ba1 = dynamic_cast<const VertexAccBias*>(_vertices[3]);
    ...
    const SO3 eR = R2.inverse() * R1 * SO3(dR);
    const Vec3d er = eR.log();
    const Mat3d invJr = SO3::jr_inv(eR);
    ...
    _jacobianOplus[0].setZero();
    // dR/dR1
    _jacobianOplus[0].block<3, 3>(0, 0) = invJr * dR.inverse().matrix();
    // dv/dR1
    _jacobianOplus[0].block<3, 3>(3, 0) = -R1_mat * SO3::hat(dv);
    // dp/dR1
    _jacobianOplus[0].block<3, 3>(6, 0) = -R1_mat * SO3::hat(dp);

    /// 残差对p1, 9x3
    // dp/dp1
    _jacobianOplus[0].block<3, 3>(6, 3) = Mat3d::Identity();
    ...
    // dp/dv1
    _jacobianOplus[1].block<3, 3>(6, 0) = dt_ * Mat3d::Identity();

    /// 残差对bg1
    _jacobianOplus[2].setZero();
    // dR/dbg1
    _jacobianOplus[2].block<3, 3>(0, 0) = invJr * SO3::jr((dR_dbg * dbg).eval()) * dR_dbg;
    // dv/dbg1
    _jacobianOplus[2].block<3, 3>(3, 0) = R1_mat * dv_dbg;
    // dp/dbg1
    _jacobianOplus[2].block<3, 3>(6, 0) = R1_mat * dp_dbg;

    /// 残差对ba1
    _jacobianOplus[3].setZero();
    // dv/dba1
    _jacobianOplus[3].block<3, 3>(3, 0) = R1_mat * dv_dba;
    // dp/dba1
    _jacobianOplus[3].block<3, 3>(6, 0) = R1_mat * dp_dba;

    /// 残差对pose2
    _jacobianOplus[4].setZero();
    // dr/dr2
    _jacobianOplus[4].block<3, 3>(0, 0) = -SO3::jl_inv(er);
    // dp/dp2
    _jacobianOplus[4].block<3, 3>(6, 3) = -Mat3d::Identity();

    /// 残差对v2
    _jacobianOplus[5].setZero();
    // dv/dv2
    _jacobianOplus[5].block<3, 3>(3, 0) = -Mat3d::Identity();
}
