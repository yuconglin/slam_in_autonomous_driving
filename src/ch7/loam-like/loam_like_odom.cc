...
void LoamLikeOdom::ProcessPointCloud(FullCloudPtr cloud) {
    ...
    // step 1. 提特征
    CloudPtr current_edge(new PointCloudType);
...
void LoamLikeOdom::ProcessPointCloudWithGround(FullCloudPtr cloud) {
    LOG(INFO) << "processing frame " << cnt_frame_++;
    // step 1. 提特征
    CloudPtr current_edge(new PointCloudType);
    CloudPtr current_surf(new PointCloudType);
    CloudPtr current_ground(new PointCloudType);
    feature_extraction_->ExtractWithGround(cloud, current_ground, current_edge, current_surf);

    if (current_edge->size() < options_.min_edge_pts_) {
        LOG(ERROR) << "not enough edge pts: " << current_edge->size();
        return;
    }
    if (current_surf->size() < options_.min_surf_pts_) {
        LOG(ERROR) << "not enough surf pts: " << current_surf->size();
        return;
    }
    if (current_ground->size() < options_.min_ground_pts_) {
        LOG(ERROR) << "not enough ground pts: " << current_ground->size();
        return;
    }
    ...
    CloudPtr ground_world(new PointCloudType);
    pcl::transformPointCloud(*current_edge, *edge_world, pose.matrix());
    pcl::transformPointCloud(*current_surf, *surf_world, pose.matrix());
...
SE3 LoamLikeOdom::AlignWithLocalMap(CloudPtr edge, CloudPtr surf, CloudPtr ground) {
    ...
    const int edge_size = edge->size();
    const int surf_size = surf->size();
    const int ground_size = ground->size();
