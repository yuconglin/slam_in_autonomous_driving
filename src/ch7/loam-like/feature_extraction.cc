...
void FeatureExtraction::Extract(FullCloudPtr pc_in, CloudPtr pc_out_edge, CloudPtr pc_out_surf, int num_scans) {
...
void FeatureExtraction::ExtractSurfAndOther(FullCloudPtr pc_in, CloudPtr pc_out_other, CloudPtr pc_out_surf,
                                            int num_scans) {
    std::vector<CloudPtr> scans_in_each_line;  // 分线数的点云
    for (int i = 0; i < num_scans; i++) {
        scans_in_each_line.emplace_back(new PointCloudType);
    ...
    for (int i = 0; i < num_scans; i++) {
        ...
        for (int j = 0; j < 6; j++) {
            ...
            ExtractFromSectorSurfAndOther(scans_in_each_line[i], sub_cloud_curvature, pc_out_other, pc_out_surf);
        }
    }
}
...
void FeatureExtraction::ExtractFromSectorSurfAndOther(const CloudPtr &pc_in, std::vector<IdAndValue> &cloud_curvature,
    ...
    for (int i = 0; i <= (int)cloud_curvature.size() - 1; i++) {
        ...
        if (std::find(picked_points.begin(), picked_points.end(), ind) == picked_points.end()) {
            ...
            pc_out_other->push_back(pc_in->points[ind]);
        }
    }
}
